...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    if (referralApplied === "true") {
      ...
      localStorage.removeItem("referralApplied");
    }
  }, []);
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get(
        `https://mn-tech-pvltd-1-0-1.vercel.app/api/users/${user._id}`,
        {
          headers: {
  ...
  const fetchReferralStats = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `https://mn-tech-pvltd-1-0-1.vercel.app/api/users/${user._id}/referral-stats`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  // ✅ Native copy function (no dependency)
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
        <div className="p-8">
          <div className="flex items-center justify-center mb-6">
            {userData?.avatar ? (
              <img
                src={userData.avatar.replace("=s96-c", "=s200-c")}
                alt="Profile"
            ...
            <div className="mt-6 border-t pt-4">
              ...
              <div className="mb-4">
                ...
                <p className="text-blue-500 font-medium break-all">
                  {`${window.location.origin}/manual-login?referral=${referralStats.referralCode}`}
                </p>
